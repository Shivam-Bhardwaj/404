...
impl BoidsSimulation {
        ...
        if let Some(ptx_path) = option_env!("BOIDS_PTX") {
            if let Ok(ptx) = std::fs::read_to_string(ptx_path) {
                ...
                for (i, b) in host_boids.iter().enumerate() {
                    hx[i] = b.x; hy[i] = b.y; hvx[i] = b.vx; hvy[i] = b.vy;
                }
    ...
    pub fn step(&mut self, dt: f32) -> Result<()> {
        // CUDA path if kernel & buffers are available
        if let (Some(ptx), Some(dx), Some(dy), Some(dvx), Some(dvy)) = (
            self.ptx.as_ref(),
            self.d_x.as_mut(),
            self.d_y.as_mut(),
            self.d_vx.as_mut(),
            ...
            // Load module/function and create stream on-demand
            ...
            dx.copy_from(&hx[..]).map_err(|e| anyhow::anyhow!("copy hx->dx: {:?}", e))?;
            dy.copy_from(&hy[..]).map_err(|e| anyhow::anyhow!("copy hy->dy: {:?}", e))?;
            dvx.copy_from(&hvx[..]).map_err(|e| anyhow::anyhow!("copy hvx->dvx: {:?}", e))?;
            dvy.copy_from(&hvy[..]).map_err(|e| anyhow::anyhow!("copy hvy->dvy: {:?}", e))?;
            ...
            // Copy back to AoS device buffer
            let mut hx = vec![0.0f32; self.num_boids];
            let mut hy = vec![0.0f32; self.num_boids];
            let mut hvx = vec![0.0f32; self.num_boids];
            let mut hvy = vec![0.0f32; self.num_boids];
            ...
            for i in 0..self.num_boids {
                host_boids[i] = Boid { x: hx[i], y: hy[i], vx: hvx[i], vy: hvy[i], species: 0 };
            }
            self.boids
